...
import { useSidebarStore, useCartStore } from '@renderer/store/store'
  ...
  useEffect(() => {
    const handleConnect = async () => {
      try {
        ...
        console.log(users)
        const user = users?.findLast((user: any) => user.walletAddress === publicKey)
  ...
  return (
    ...
    >
      <NavigationMenuList>
        ...
        <NavigationMenuItem>
          <Link to="/">
            <NavigationMenuTrigger className="tracking-tighter">Home</NavigationMenuTrigger>
          </Link>
        </NavigationMenuItem>
        <NavigationMenuItem>
          <Link to="/market">
