...
import { useButtonStore } from '@renderer/store/store'
...
import { p2pCall } from '@renderer/api/games/p2pcall'
...
export default function GameGrid({ onClick }: any) {
  ...
  const transformData = (
    ...
    switch (buttonIndex) {
      ...
      case 1:
        return data.sort((a, b) => b.rating - a.rating && b.rating_count - a.rating_count)
      case 2:
        return resellGames
    }
  }
  ...
  const handleSelling = async ({ name, id }) => {
    const myPromise = new Promise(async (resolve, reject) => {
      try {
        ...
        const buy_response = await axios.post('http://localhost:3001/buy', bodyLicense)
  ...
  return (
    <div className="flex flex-col w-full  mb-12 items-center justify-center">
      {isLoading ? (
        <SkeletonGrid />
      ) : (
        <div className="grid grid-rows-2 grid-cols-2 md:grid-rows-2 xl:grid-cols-4    gap-5 gap-y-8  place-items-center">
          {slicedData?.map(
            (
              element: {
                id: number
              ...
              <div onClick={() => onClick(element)}>
                <Games
                  ...
                  className={'w-4/5 rounded-md border cursor-pointer h-[550px] '}
                />
              </div>
            )
          )}
