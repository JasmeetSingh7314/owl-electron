...
import { Dialog, DialogContent, DialogFooter, DialogTrigger } from '@renderer/components/ui/dialog'
...
export default function ProfileHeader() {
  ...
  const userInfo = JSON.parse(sessionStorage.getItem('current-user') as string)
      ...
      <div className="flex flex-col space-y-4 md:space-y-0 md:space-x-6 md:flex-row">
        ...
        <div className="flex flex-col mx-12 w-full">
          ...
          <span className="flex gap-x-12">
            {userInfo?.tags?.map(
              (tag: string | number | boolean | undefined, id: Key | null | undefined) => (
                <span
                  key={id}
