import { useChatStore, useSidebarStore } from '@renderer/store/store'
import { users } from '@/utils/dummydata/chatdummyDb'
import { dummyStats } from '@renderer/utils/dummydata/dummyGameData'
  ...
  return (
    <section className="w-full flex bg-white/[0.045] backdrop-blur-lg rounded-lg px-12 py-24">
        ...
        <ul className="list-disc flex flex-col gap-y-4 mt-12">
          {[...Array(5)].map((_, id) => (
            <li key={id} className="flex gap-x-2 items-center ml-24">
              <span className="text-xl tracking-tight   font-inter">{dummyStats[id]}:</span>
      ...
      <div className="flex-1 ml-24">
        <span className="text-3xl font-semibold font-urbanist text-white">Groups</span>
