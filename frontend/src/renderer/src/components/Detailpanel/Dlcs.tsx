...
import { SectionHeader } from '../desktop/GameCard'
...
export default function DlcsComp({ detailsResponse, id }: any) {
  ...
  return (
    <div>
      {!dlcData.data || (dlcData.data.length === 1 && dlcData.data[0].status === 400) ? (
        <div className="font-jura text-2xl">No DLC's available for this game.</div>
      ) : (
            ...
            {dlcData?.data?.map(
              (
                ...
                <AccordionItem
                  ...
                  startContent={<Avatar color="primary" src={dlc?.cover?.url} />}
                >
